...
Página separada: lista detalhada de licitações.

Instalar:
    pip install streamlit pandas openpyxl plotly gspread google-auth

Rodar:
    streamlit run dashboard_licitacoes_streamlit.py
"""
...
LOGO_SVG = """
<img src="https://www.ampla.com.br/wp-content/uploads/2022/12/logo-ampla.svg" height="105" style="margin-bottom: 15px; margin-top: 5px;">
"""
LOGO_SVG_BRANCA = """
<img src="https://handson.tec.br/static/img/logo/logo-branca.png" height="50">
"""
...
section[data-testid="stSidebar"] [data-baseweb="select"] svg {{ fill: white !important; color: white !important; }}
...
section[data-testid="stSidebar"] [data-baseweb="select"] {{
    background: rgba(255,255,255,0.12) !important; border-radius: 8px !important;
...
[data-testid="metric-container"] {{
    background: {SURFACE} !important; border: 1px solid {BORDA} !important;
    border-radius: 14px !important; padding: 1.1rem 1.3rem !important;
    border-top: 3px solid {AZUL} !important;
}}
[data-testid="stMetricLabel"] {{
    font-size: 10px !important; text-transform: uppercase !important;
    letter-spacing: 0.1em !important; color: {MUTED} !important; font-weight: 600 !important;
}}
[data-testid="stMetricValue"] {{
    font-weight: 700 !important; font-size: 1.85rem !important;
    color: {AZUL} !important; letter-spacing: -0.02em !important;
}}
[data-testid="stMetricDelta"] {{ font-size: 11px !important; color: {MUTED} !important; }}
[data-testid="stMetricDelta"] svg {{ display: none !important; }}

.stButton > button {{
    background: {AZUL} !important; color: white !important; border: none !important;
    border-radius: 8px !important; font-weight: 600 !important;
    font-family: 'Space Grotesk', sans-serif !important; letter-spacing: 0.02em !important;
}}
.stButton > button:hover {{ opacity: 0.82 !important; color: white !important; border: none !important; }}

[data-testid="stDataFrame"] {{ border-radius: 12px !important; overflow: hidden !important; border: 1px solid {BORDA} !important; }}
[data-testid="stDataFrame"] * {{ font-family: 'Space Grotesk', sans-serif !important; background-color: {SURFACE} !important; color: {TEXTO} !important; }}
...
.section-header {{
    ...
    letter-spacing: 0.12em !important; color: {MUTED} !important; margin-bottom: 0.75rem !important;
}}
.status-ok {{
    display: inline-flex; align-items: center; gap: 6px;
    background: rgba(0,31,255,0.12); color: {AZUL};
    border: 1px solid rgba(0,31,255,0.25); border-radius: 20px;
    ...
    if "gc" not in st.session_state:
        ...
        if "credentials" in st.secrets:
            ...
            st.error(
                ...
                "No Streamlit Cloud: Settings → Secrets → cole o bloco [credentials].\n"
                f"Localmente: coloque credentials.json em {CREDENTIALS_PATH}"
            )
            st.stop()
...
with st.sidebar:
    ...
    with col_s2:
        atualizar = st.button("↻", help="Recarregar dados")
...
if not sheets_ok:
    st.markdown(
        ...
        <div style='text-align:center;padding:4rem 2rem'>
          {LOGO_SVG}
          <div style='font-size:1.4rem;font-weight:800;color:{AZUL};margin:1.5rem 0 0.5rem'>
            Erro ao conectar ao Google Sheets
          </div>
          <div style='background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);
                      border-radius:12px;padding:1rem 1.5rem;max-width:560px;
                      margin:1rem auto;text-align:left;font-size:13px;color:#fca5a5'>
            {sheets_err}
          </div>
          <div style='color:{MUTED};font-size:13px;margin-top:1rem'>
            Verifique o credentials.json e as APIs ativadas no Google Cloud.
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
if df_raw.empty:
    st.markdown(
        ...
        <div style='text-align:center;padding:4rem 2rem'>
          {LOGO_SVG}
          <div style='font-size:1.4rem;font-weight:800;color:{AZUL};margin:1.5rem 0 0.5rem'>
            Planilha conectada, mas ainda sem dados
          </div>
          <div style='color:{MUTED};font-size:13px'>
            Rode o buscador para popular a planilha "Data Licitacoes".
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
# Salvar no session_state para a outra página acessar
...
with col_logo:
    ...
    st.markdown(
        ...
        f"Radar de Licitações · Setor de Publicidade</div>",
        unsafe_allow_html=True,
    )
with col_status:
    st.markdown(
        "<div style='text-align:right;margin-top:8px'>"
        "<span class='status-ok'><span class='dot-ok'></span>Sheets</span></div>",
        unsafe_allow_html=True,
    )
...
c1.metric("Editais",
          f"{len(df):,}".replace(",", "."),
          delta=f"de {len(df_raw):,}".replace(",", ".") + " total")
c2.metric("Score Alto ≥70", altos,
          delta=f"{altos / max(len(df), 1) * 100:.0f}% filtrado")
c3.metric("Score Médio", medios)
c4.metric("Valor estimado",
          f"R$ {valor_total / 1e6:.1f}M" if valor_total > 0 else "—")
...
with col_g1:
    ...
    fig_uf = px.bar(
        uf_counts, x="Qtd", y="UF", orientation="h",
        color="Qtd",
        color_continuous_scale=[SURFACE2, AZUL_MID, AZUL],
    ...
    fig_uf.update_layout(
        ...
        yaxis=dict(categoryorder="total ascending",
                   tickfont=dict(size=11, color=MUTED), gridcolor=SURFACE2),
...
with col_g2:
    ...
    colors = [FONTE_CORES.get(f, MUTED) for f in fc["Fonte"]]
    fig_pie = go.Figure(go.Pie(
        labels=fc["Fonte"], values=fc["Qtd"],
        hole=0.62,
        marker_colors=colors,
        textinfo="label+percent",
        textfont=dict(size=11, family="Space Grotesk", color=TEXTO),
...
with col_g3:
    ...
    pri = df["prioridade"].value_counts()
    for label, count in pri.items():
        ...
        st.markdown(
            ...
            f"<span style='font-size:12px;font-weight:700;color:{cor}'>{count}</span>"
            f"</div>"
            f"<div style='height:7px;background:{SURFACE2};border-radius:4px;overflow:hidden'>"
            f"<div style='height:100%;width:{pct:.0f}%;background:{cor};border-radius:4px'></div>"
            f"</div>"
            f"<div style='font-size:10px;color:{MUTED};margin-top:2px'>{pct:.0f}% do total</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
...
with col_top:
    ...
    for _, r in df.nlargest(10, "score").iterrows():
        ...
        link_html = (
            ...
            f'style="font-size:10px;color:{AZUL};text-decoration:none;font-weight:600">'
            f'Ver edital →</a>'
        ) if link else ""
        enc_html = f"<span>⏱ {enc}</span>" if enc else ""

        st.markdown(
            f"<div style='background:{SURFACE};border:1px solid {BORDA};"
            ...
            f"<div style='font-size:12.5px;font-weight:500;line-height:1.5;"
            f"flex:1;color:{TEXTO}'>{objeto}{'...' if len(objeto)==130 else ''}</div>"
            f"<div style='display:flex;flex-direction:column;align-items:flex-end;"
            f"gap:4px;flex-shrink:0'>"
            f"<span style='background:{cor}18;color:{cor};border:1px solid {cor}30;"
            f"border-radius:5px;padding:2px 9px;font-size:11px;font-weight:700'>{sc}</span>"
            f"{link_html}"
            f"</div></div>"
            f"<div style='display:flex;flex-wrap:wrap;gap:10px;margin-top:6px;"
            f"font-size:11px;color:{MUTED}'>"
            f"<span>📍 {r.get('uf','') or '—'}</span>"
            f"<span>🏛️ {orgao}</span>"
            f"<span>📰 {r.get('fonte','') or '—'}</span>"
            f"<span>💰 {valor_str}</span>"
            f"{enc_html}"
            f"</div></div>",
            unsafe_allow_html=True,
        )
...
with col_kw:
    ...
    if kw_top:
        ...
        for kw, cnt in kw_top:
            ...
            st.markdown(
                ...
                f"<span style='font-size:11px;color:{MUTED};font-weight:600'>{cnt}</span>"
                f"</div>"
                f"<div style='height:5px;background:{SURFACE2};border-radius:3px;overflow:hidden'>"
                f"<div style='height:100%;width:{pct:.0f}%;"
                f"background:linear-gradient(90deg,{AZUL_MID},{AZUL});border-radius:3px'></div>"
                f"</div></div>",
                unsafe_allow_html=True,
            )
    ...
    if "data_pub" in df.columns:
        por_dia = (
            df.dropna(subset=["data_pub"])
            .groupby(df["data_pub"].dt.date)
            .size()
            .reset_index(name="n")
            .tail(20)
        )
        ...
        fig_line = px.line(
            por_dia, x="Data", y="Editais",
            template="plotly_dark",
            color_discrete_sequence=[AZUL],
        )
        fig_line.update_traces(
            line_width=2, mode="lines+markers",
            marker=dict(size=4, color=AZUL),
        )
